    ...
    foreach ($phones as $key => $value) {
     ...
     `deleted` = 0  order by `id` desc",[(int)$value ,$_POST['vote']]);
      ...
      if(empty($phonesRow)){
        ...
        if(1 == $votes['with_url']){ $urlExt = 'https://54.37.80.11/v/'.$_POST['vote'].'/'.$value; }
...
foreach ($phones as $key => $value) {
  ...
  $msg = $votes['msg'].'https://54.37.80.11/v/'.$_POST['vote'].'/'.$value['phone'];
...
foreach ($phones as $key => $value) {
  if(!empty($value)){
    ...
    echo '<div class="alert alert-warning c">تم إرسال الى : '.$value.'</div>';
    $msg = $votes['msg'].'https://54.37.80.11/v/'.$_POST['vote'].'/'.$value;
     ...
     'هل تريد المتابعة؟',
      function(){
          ...
          if($('.vote-form')[0].checkValidity()) {
            $.post("https://54.37.80.11/v/api",data,function(result){
    ...
    'تأكيد العملية',
     'هل تريد المتابعة؟',
      function(){
            ...
            $.post("https://54.37.80.11/v/api",{type:'delete-vote',id:id},function(result){
...
function getPhone(page ,limit_pp){
  ...
  $.post("https://54.37.80.11/v/api",{page:page,limit_pp:limit_pp,type:"get-phone"},function(result){
...
$(document).on('click','.export',function(){
  ...
  location.replace('https://54.37.80.11/v/export/?'+data);
...
$(document).on('click','.admin-note-change',function(){
  ...
  $.post("https://54.37.80.11/v/api",{type:'admin-note-change',id:id,val:val},function(result){
     ...
     'هل تريد المتابعة؟',
      function(){
          ...
          if($('.'+target)[0].checkValidity()) {
            $.post("https://54.37.80.11/v/api",{send_type:send_type,vote:vote,single_phone:phone1,group_phone:phone2,single_name:name1,group_name:name2,t:t,type:"insert-phone"},function(result){
