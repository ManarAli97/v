...
# SELECT
# $res = $DB->query("SELECT * FROM user WHERE user_name=:name and user_pass=:pass",array('name'=>'aws','pass'=>'aws'));
# $res =$DB->query("SELECT * FROM fruit WHERE user_name=? and user_pass=?",array('aws','aws'));

# DELETE
# $res =$DB->query("DELETE FROM fruit WHERE id = :id", array("id"=>"1"));
# $res =$DB->query("DELETE FROM fruit WHERE id = ?", array("1"));

# Update
# $res =$DB->query("UPDATE fruit SET color = :color WHERE name = :name", array("name"=>"strawberry","color"=>"yellow"));
# $res =$DB->query("UPDATE fruit SET color = ? WHERE name = ?", array("yellow","strawberry"));

# Insert
# $res =$DB->query("INSERT INTO fruit(id,name,color) VALUES(?,?,?)", array(null,"mango","yellow"));//Parameters must be ordered
# $res =$DB->query("INSERT INTO fruit(id,name,color) VALUES(:id,:name,:color)", array("color"=>"yellow","name"=>"mango","id"=>null));//Parameters order free

# get last ID
# $res =$DB->lastInsertId();

# close connection
# $res =$DB->CloseConnection;
...
class DB
{
	...
	private function Connect()
	{
      ...
      echo "<br>Err#08 connection err1<br>";
